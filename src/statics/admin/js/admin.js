...
function fullscreen() {
	...
	if (scr.length > 0 && !scr.attr('disabled')) {
		...
		$('#fullScreen').removeClass('admin_unfull').addClass('admin_full');
		return;
	}
	if (scr.length == 0) {
	...
	$('#fullScreen').removeClass('admin_full').addClass('admin_unfull');
	return false;
}
/**
 * 提交页面搜索信息
...
function init() {
	...
	$('#B_main_block a').each(function(index) {
		var name = $(this).attr('href').substr($(this).attr('href')
				.indexOf('#')
				+ 1);
		if (name == 'modemanage') {
...
function findItem(obj,name){
	var items = null, ret = null;
	...
	if (ret == null && items != null) {
		return findItem(obj[i]["Items"], name);
	}
	return ret
}
/**
...
function showMenu2(target) {
	...
	if (name == 'common') {
		...
		data = findItem(SUBMENU_CONFIG, name);
		title = target.html();
	}
	...
	$.each(data.Items, function(index, o) {
				...
				dl.append(dt);
				if (o.Items) {
					var dd = $('<dd style="display: none;"></dd>'), ul = $('<ul></ul>');
					dd.append(ul);
...
function openWin(mid, url, nickname) {
	...
	if (mid) {
		...
		if (cls.length > 1) {
			...
			ob = findItem(findItem(SUBMENU_CONFIG, cls[0]).Items, cls[1]);
	...
	if (mid == 'default') {
		...
		if (cls.length > 1) {
			html = '<span>当前位置</span><em>&gt;</em><span class="admenu_down" data-id="">'
					+ findItem(MAIN_BLOCK, cls[0]).name + '</span>';
			arBread.html(html);
			ob = findItem(findItem(ob, cls[0]).Items, cls[1]);
			html += '<em>&gt;</em><span class="admenu_down" data-id="' + cls[0]
					+ '">' + ob.Name + '</span>';
			arBread.html(html);
		}
